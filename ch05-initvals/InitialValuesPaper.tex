...
The output signal is disturbed by white random noise, resulting in noisy error-prone data. It is also possible to apply the estimation procedure in this chapter to a system that is disturbed by colored noise, as will be demonstrated in Section~\ref{se:ExpMeas}. 
...
The true model $\true{G}$ as an initial estimate is the best possible initial estimate one can use for the nonlinear identification algorithm (the leftmost path in \figref{fig:flowgraph}) and will allow the optimization to converge to the \emph{best local optimum}, i.e. $\model{0}$.
\end{conjecture}
...
However, such an approximation is overly optimistic when $n_{\mathrm{success}}$ approaches the boundaries of its domain (i.e. when $\eta \approx 0$ or $\eta \approx 1$) since normal confidence regions are always symmetrical whereas the actual success rate is inherently bounded as $0 \leq \eta 1$~\citep{Ross2003}.
An alternative method that does not approximate the binomial distribution by a normal one, is the Clopper-Pearson method~\citep{Clopper1934} (as implemented in \MATLAB's \code{binofit}).
...
\begin{align}
 ...
 \underline{\eta} &\isdef 
  \mathrm{CDF}_B^{-1}(\alpha/2; n_{\mathrm{success}}, N_{\MC} - n_{\mathrm{success}} + 1)\\
  \overline{\eta} &\isdef
  \mathrm{CDF}_B^{-1}(1-\alpha/2; n_{\mathrm{success}} + 1, N_{\MC} - n_{\mathrm{success}})
\end{align}
where $\mathrm{CDF}_B^{-1}(\alpha; n,m)$ denotes the inverse \gls{CDF} of a $\mathrm{Beta}(n,m)$ distribution that is closely related to the Binomial distribution.
...
However, since there is often a clear separation between the different techniques, no efforts have been made to try out confidence interval estimators that offer more conservative results.
...
\begin{figure}
  \centering
  \includegraphics[width=0.6\onecolumnwidth]{\thisDir/fig/measurement.pdf}
...
To determine how important it is to select the 
\TODO{justify these experiments}
...
In \figref{fig:init:histogramEnhancement}, the different methods are compared in this respect by subtracting the performance of the $\model{best}$ from the performance of the respective methods.
...
\end{itemize}
discrete-time filters (see \figref{fig:bodeplots} and \tabref{tbl:init:stresstest}) of tenth degree generated by the MATLAB functions \texttt{cheby1}, \texttt{cheby2}, \texttt{ellip} and \texttt{butter} and an SNR of $20 \unit{dB}$ and $N=1024$ measured samples.
All filters $G$ have been normalized in such a way that $\norm[2]{G} = 1$.
...
Consequently, the ease-of-use of parametric model fitting can be increased considerably by making it possible to obtain good parametric models without requiring any additional user interaction.

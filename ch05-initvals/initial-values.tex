...
\end{conjecture}
\TODO{Johan: merk op dat dit niet zo hoeft te zijn! zelfs vanuit $\true{G}$ kan het misgaan}
...
\TODO{Johan: beter een relatief criterium gebruiken (delen door $\norm{\true{G}}$)}
...
\TODO{describe settings for the experiment: $N$, etc.}

The observed timings are shown in \TODO{tableref}.
\TODO{discuss timings}

\TODO{table summary}

\subsection{Asymptotic runtime}
For the asymptotic runtime, we are mainly interested in how each of the smoothers scale with the size of the dataset, i.e. the number of samples $N$.
...
Since the \gls{RFIR} relies on fitting a \gls{FIR} filter to $N$ datapoints, this relies on inverting a matrix with largest dimension $N$. 
Using general methods, this is an operations that requires $\bigO{N^3}$ time.
% However, due to the Toeplitz structure~\citep{Heinig2011} of the regression matrix, this can be solved in $\bigO{N^2}$.

It can be seen that the local modeling methods scale more favorably than e.g. \gls{RFIR}.

\subsection{Implementation aspects}
\TODO{hint towards parallization}
\TODO{most costly: iteration}

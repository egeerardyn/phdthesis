...
\label{sec:initvals}
\chapterBasedOn{Geerardyn2015TIM}
...
\begin{figure}
 ...
 \caption[Model error for different initialization schemes on simulations.]{The distance $\|\model{\bullet} - \model{\true{}} \|_2$  between the different estimates ($\model{exist}$~\legref{leg:dist-G-exist}, $\model{trunc}$~\legref{leg:dist-G-trunc} and $\model{RFIR}$~\legref{leg:dist-G-RFIR} ) and the `best theoretical' estimates ($\model{\true{}}$) shows that the smoothers help the estimate to converge to $\model{\true{}}$.
 ...
 $\norm[2]{\true{G} - \model{\true{}}}$\legref{leg:dist-G0} shows how far the true model and `best theoretical' estimates are apart to put the other distances into perspective. }
...
\begin{figure}
  ...
  \caption[Performance penalty for selecting the second best model.]{

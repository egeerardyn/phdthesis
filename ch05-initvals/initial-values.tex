...
\label{DFTspectra}
\end{align}
with $q^{-1}$ sampled in  $q_k^{-1} = e^{-j\omega_k}$, where $\omega_k = \frac{2\pi k}{N}$ are the DFT frequencies, and $I$ is a polynomial of order $\max(N_a,N_b) - 1$, which depends on the difference between the initial and end conditions of the filter $\true{G}$ during the measurement record.
\begin{assumption}
...
\TODO{spectra als formule geven}
\end{remark}
%}
...
The distance measure used in equation \eqref{eq:successrateTol} is defined as
\begin{align}
...
\begin{enumerate}
  ...
  \item `poor' estimates which are much farther from $\model{\trueSymbol}$.  This is caused by the local optima.
\end{enumerate}
\item
...
Instead, the typical \code{tic} and \code{toc} functions measure the wall-clock time.
...
In this case, the \MATLAB function \code{timeit}~\citep{matlab:timeit} has been used to measure the timing of the different functions.
...
\end{itemize}
The median timings as returned by \code{timeit} are reported in \tabref{tbl:init:timing}.
...
For longer datasets, it appears that the \code{kron} function is one of the major culprits for the long runtimes of the \gls{LPM}.

...
In \secref{sec:lrmhinf:robust-control-basics}, a short introduction to robust control is linked to the work in this chapter.
...
\section{Intermezzo: Basics of Robust Control}
\label{sec:lrmhinf:robust-control-basics}
When designing a feedback controller $\Controller$ (as in \figref{fig:lrmhinf:tpc:nominal}), the goal is to design $\Controller$ such that the output $y$ of a plant $\true{\Plant}$ follows the command signal $r_2$ with some additional constraints on the performance of the whole closed-loop system (e.g. limited overshoot, short response times, good disturbance suppression, limited controller effort, \ldots).
...
\begin{figure}[h]
 \centering
 \input{\thisDir/figs/tpc.tikz}
 \caption[Closed-loop block schematic.]{The considered feedback configuration $\ClosedLoop{\true{\Plant},\Controller}$.}
...
\begin{figure}%[10]{R}[1em]{5cm}
  \centering
  \input{\thisDir/figs/modelset.tikz}
  \caption{The plant model set.}
...
Hence, the problem is then to construct the model set $\ModelSet[]$ as illustrated in \figref{fig:lrmhinf:modelset}.
The design process thus guarantees performance and stability for the whole set instead of for a single model.
However, these guarantees hinge on proper specification of $\ModelSet[]$ which is typically done by estimating a nominal model $\estimated{\Plant}$ and the model uncertainty $\Delta(\estimated{\Plant}, \true{\Plant})$.
A generic way to express the plant model set (see also \figref{fig:lrmhinf:modelset}) is
\begin{equation}
  \ModelSet[]_{\mathrm{generic}} 
  \isdef 
  \Set{ \Plant | \norm{\Delta(\estimated{\Plant}, \Plant)} \leq \gamma}
  \equiv
  \Set{ \Plant(\estimated{\Plant}, \Delta) | \norm{\Delta} \leq \gamma}
  \label{eq:lrmhinf:model-set:generic}
\end{equation}
where both the norm and the concrete function $\Delta(\estimated{\Plant}, \Plant)$ are a choice for the control engineer.
An example of such a model set is easily imagined for additive uncertainty ($\Plant(\estimated{\Plant}, \Delta) = \estimated{\Plant} + \Delta$) when a $\Hardy_{2}$ norm is used, in that case one obtains
\begin{equation}
  \ModelSet[]_{\mathrm{additive,2}} = \Set{ \Plant | \norm[2]{\estimated{\Plant} - \Plant} \leq \gamma}
  \text{,}
\end{equation}
which is a multidimensional ball with radius $\gamma$.
...
$\ClosedLoop{\Plant,\Controller}$ denotes the closed loop transfer function from the loop inputs $\mat{r_2 & r_1}^{\TT}$ to the output and input of the plant $\mat{y & u}^{\TT}$ as schematically in \figref{fig:lrmhinf:tpc}.

...
Recent trends in many disciplines have produced increasingly complicated systems.
Consequently, building a full-complexity model rapidly becomes disproportionately expensive for a required model fidelity.
...
\end{equation}
If the measurement time is finite, non-parametric approaches only give access to a finite and discrete grid of frequencies $\omega_k \in \DFTGrid $, i.e. $\DFTGrid \isdef{\omega_1, \omega_2, \ldots, \omega_{N-1}}$.
...
However, the expected value of  $\gamma_{\mathrm{FRF}}$  for different realizations of the input signal and noise for a fixed measurement time, underestimates the actual peak gain $\gamma$, i.e. $\expectedValue{\gammaAtGrid} \leq \gamma$.
...
\begin{guideline}[Measure at least $5\timeconst\,$ to observe its peak value using the local models]
...
\begin{figure}[p]
  ...
  \hfill
  \photobox{\includegraphics[width=\figurewidth]{\thisDir/figs/avis-motors-cropped.jpg}}
  %TODO: right figure is just a tad too small
...
In addition, $u_{\Delta}$ is noise free under standard assumptions~\citep{Hansen1989}, hence the method of \secref{sec:lrmhinf:LPMHinf} applies directly.
...
A reliable local parametric \Hinf{} norm estimation technique for high performance, non-conservative robust control design is presented.

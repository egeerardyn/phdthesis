  \begin{itemize}
    \item \TODO{add section from Benchmark paper}
    \item \TODO{Hinf macro aanpassen voor PDF support (indien dit niet conflicteert met glossaries)}
  \end{itemize}
...
In the present paper, these methods are extended to provide both an at-grid and inter-grid evaluation of the maximum singular value near resonances, as is required for the $\Hinf{}$ norm and consequently the design of robust controllers.
...
\begin{equation}
  \gamma = \infnorm{\Delta} \isdef \sup_{\omega} \abs{\Delta(\omega)} \text{ for \gls{SISO} systems}
  \text{.}
...
\begin{remark}
It is even more common to use a weighted norm, e.g.  $\infnorm{W \Delta V}$ in robust control designs for some weighting filters $(W,V)$ that are stable and minimum-phase.
Two cases can be distinguished: either these filters are chosen by the control engineer and hence known, or, one wishes to construct such filters.
...
As such, \eqref{eq:lrmhinf:HinfNormSISO} can be rewritten as
\begin{equation}
...
The main contribution of this chapter is to  estimate the inter-grid term $\gamma_{\mathrm{IG}}$ without the need for a (global) parametric model.
...
One can implement the definition~\eqref{eq:lrmhinf:gamma-inter-grid} of $\gamma_{\mathrm{IG}}$ directly.
...
To allow inspection of $\abs{\Delta(\omega)}$ in~\eqref{eq:lrmhinf:HinfNormSISO} for arbitrary values of $\omega \in \Omega$, we propose an interpolation approach to estimate $\gamma$ and to allow visual inspection of $\abs{\Delta(\omega)}$.
...
\end{align}
Using the $k_L(\omega)$ and $k_R(\omega)$ functions, two candidate values for $\abs{\Delta(\omega)}$ are obtained:lrmhinf: $\abs{\LocalModel{\Delta}_{k_L(\omega)}(\omega)}$ and $\abs{\LocalModel{\Delta}_{k_R(\omega)}(\omega)}$.
...
Concretely, any proper real-rational system $\Delta$ can be decomposed in a parallel connection of lower-order sub-systems by means of a partial fraction expansion as in \eqref{eq:excitation:PFE}.
For clarity, we repeat the expression here again:
It is well-known in the mechanical engineering community (see e.g. \citet{Gawronski2004}), that around re
 and using the same logic as in \chapref{sec:excitation}.
...
To estimate $\infnorm{\Delta}$ mainly the resonant sub-systems are of interest.
...
It can be observed for $\gamma_{\mathrm{IG}}$ in \figref{fig:lrmhinf:exampleIntergrid} that the \gls{LRM} is able to model the resonance well and yields a useful estimate $\infnorm{\Delta_{\mathrm{LRM}}} = 24$ in the simulation.
...
\begin{figure}
  ...
  \caption[Estimated $\gamma$ versus measurement time.]{The $\Hinf{}$ norm~\legref{leg:runlength-true} is underestimated significantly for short experiments ($N<20\timeconst$) by $\expectedValue{\gamma_{\mathrm{FRF}}}$~\legref{leg:runlength-frf} and the \gls{LPM}~\legref{leg:runlength-LPM}.
...
It can be seen in \figref{fig:lrmhinf:MC-run-length} that $N \geq 20\timeconst$ is required to attain a frequency grid that is dense enough to obtain a reliable $\gamma_{\mathrm{FRF}}$, i.e. $\expectedValue{\gamma_{\mathrm{FRF}}} \approx \gamma$.
...
E.g. \citet{Scheid1995} describes a method to produce parametric overbounds, but notes in Section~IV that limited frequency resolution can hamper construction of these filters.
...
In this paper, an eighth-order $\estimated\Plant$~(\figref{fig:lrmhinf:avis-frf}) is estimated.
...
\begin{figure}
 ...
 \caption[\Glsentryname{FRF} and nominal model of \glsentryname{AVIS}.]{Measured \gls{FRF} \legref{m2t:plantFRF} and estimated parametric model $\hat{P}$ \legref{m2t:plantModel} of the \glsfirst{AVIS}.}
...
\begin{figure}
 ...
 \caption[$\Delta$ of the \glsentryname{AVIS}, estimated using local modeling (detail).]{Measured $\Delta$ for the \gls{AVIS} using the \gls{LRM}~\legref{leg:lrmhinf:avis4-LRMi} and the proposed interpolation. Clearly, the \gls{LRM} leads to a higher peak value~\legref{leg:lrmhinf:avis4-LRM-hinf}, especially at $328\unit{Hz}$. The validity of the LRM local parametric model is confirmed by the validation measurement at the dense frequency grid~\legref{leg:lrmhinf:avis4-validation}, which reveals excellent interpolation properties and a similar peak value~\legref{leg:lrmhinf:avis4-valid-hinf}.}
...
The proposed method \eqref{eq:lrmhinf:gammaInterpolOptim} yields $\gamma_{\mathrm{IG}}=-2.5\unit{dB}$.

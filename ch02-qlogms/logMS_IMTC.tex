...
\subsection{Multisine Excitations}
  To excite the system under test we will use multisine excitations \citep{Pintelon2001} with $F$ frequency components:
  \begin{equation}
  ...
  \begin{equation}
    ...
    \text{,}
  \end{equation}
  where the least common (integer) multiple of the real values $T_k$ is defined as
  \begin{equation}
    \lcm \set{T_1, \ldots, T_F} \isdef
      \min \set{T \given{\forall i \in \set{1,\ldots,F}, \exists n_i \in \NaturalNumbers: n_i T_i = T }}
      \text{.}
   \end{equation}
  Note that this minimum doesn't always exist (e.g. when $f_1 = 1 \unit{Hz}$ and $f_F = \pi \unit{Hz}$), in such cases the multisine is aperiodic.

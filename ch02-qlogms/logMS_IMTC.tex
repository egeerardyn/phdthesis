...
\section{Introduction} \label{sec:intro}
  It often occurs that we wish to measure the frequency response function (FRF) of a linear time invariant (LTI) system in a frequency band that covers several decades \cite{bragos2001characterisation,sanchez2011optimal,niedostatkiewicz2009,VanGheem2004}.
  Of course, we also want to attain a specified uncertainty level over the full frequency span.

  Any LTI system of finite order can be expanded into a  finite sum of systems of first and second order by means of a partial fraction expansion\cite{Oppenheim1983}.
  ...
  This problem was already discussed in \cite{GeerardynIMTC2012}.
...
\section{Methodology} \label{sec:method}
  ...
  the amplitude of the transfer function \cite{Oppenheim1983}:
  \begin{align}
...
\subsection{Multisine Excitations}
  To excite the system under test we will use multisine excitations \cite{Pintelon2001} with $F$ commensurate frequency components:
  \begin{equation}
...
\subsection{Equidistant (Linear) Grid Multisine}
  In many applications, an equidistant frequency grid is used for $u\left( t\right) $ because this fits best with the classical DFT analysis \cite{OppenheimDT,Mandal2007}. It allows to analyze periodic signals on an equidistant frequency grid very efficiently.
...
\subsection{Quasi-Logarithmic Grid Multisine}
  ...
  \end{equation}
  Such a grid is called a quasi-logarithmic (quasi-log) grid \cite{Pintelon2001}. 
...
\subsection{Compensated Quasi-Logarithmic Grid Multisine}
  ...
  In \cite{Rojas2007} it is suggested that band-limited $1/f$ noise is a reasonable, albeit not optimal, robust excitation signal.
  ...
  \begin{figure}[ht]
    ...
    \caption{Comparison of the amplitude spectrum of a linear grid multisine, a logarithmic grid multisine, the compensated quasi-logarithmic multisine and the `pink' quasi-log multisine.}%
    \label{fig:freqGrids}
  \end{figure}
...
\section{Optimization of the frequency spacing} \label{sec:optimAlpha}
  ...
  The covariance matrix $\C_{G\left(\omega_{\interest}\right)}$ of the parametric transfer function, which is pretty hard because the common lack of intuition about this model given the parameters, can be shown \cite{Pintelon2001, Gallager2008, matrixcookbook} to be approximately equal to
  \begin{align}
  ...
  \begin{equation}
    ...
    \label{eq:FisherInfo}
  \end{equation}
  the Fisher information matrix \cite{Pintelon2001}.
  ...
  By assuming that the $X\left( \omega \right) $ are independently distributed over the frequency $\omega$, the variance-covariance matrix $\C_{X}$ is a diagonal matrix \cite{Pintelon2001}:
  \begin{equation}
    \C_{X} =
               \left[ \left.
               \begin{matrix}
  ...
  The grid resolution in a (quasi-) log grid is $\left( \alpha - 1\right)\omega_k$ while the bandwidth is approximately $2\damping\wn$.
  ...
  \begin{figure}
    ...
    \caption{Evaluation of the variance $\sigma_G^2\left( \wM \right) $ for different prototype systems as function of the misalignment $\beta$.
  ...
  \begin{figure}[ht]
    ...
    \caption{Worst case variance of the peak in the transfer function for logarithmic
  ...
  \begin{figure}%
    ...
    \caption{Depiction of the transfer function $G$ of a second order system, along with a logarithmic multisine excitation $U$ for $\alpha = \alpha^{\star}$. The circled sine components lie within the $3 \unit{dB}$ bandwidth of $G$.}%
...
In the following set of simulations, we will study how the FRF of different second order systems with equal damping (and the corresponding uncertainty) are measured by the multisine excitation signals from Section~\ref{sec:method}.
  ...
  This form can also be obtained by applying Tustin's method or bilinear transform \cite{Oppenheim1983} to the continuous time system \eqref{eq:secondOrderSystemCT}.
  ...
  The obtained FRF is then fitted to the transfer function model \eqref{eq:secondOrderSystemDTidentifiable} using the \texttt{elis} function of the \FDIDENT \cite{FDIDENT} for \matlab.
  ...
  \begin{figure}[th]
    ...
    \caption{FRF of a second order system with $\fR=40\unit{Hz}$ and $\damping=0.2$
             along with the corresponding variance and bias.}%
    \label{fig:FRF1}
  \end{figure}
  ...
  \begin{figure}%
    ...
    \caption{FRFs for different second order systems with constant damping
...
\section{Measurements} \label{sec:measurement}
  \subsection{Setup}
    ...
    According to its manual \cite{datasheet_bk1613}, a $0.5\unit{dB}$ ripple with three resonances is present in the pass-band and the $3\unit{dB}$ break frequencies are located at $\frac{\fc}{\sqrt{2}}$ and $\fc \sqrt{2}$.
    A first guess to realize such a filter is a third-order Chebyshev band-pass filter of Type I \cite{Zverev1967} for which the poles have a damping that is larger than $0.09$.
    ...
    \end{equation}
    was estimated using \texttt{elis} \cite{FDIDENT}.
    The initial estimate was a third-order band-pass type I Chebyshev filter \cite{Zverev1967} that conformed to the information given in the datasheet \cite{datasheet_bk1613}.
    ...
    \caption{Measured FRFs of the different \bruelkjaer BK1613 octave filters together with the observed variance for a linear, quasi-log and pink quasi-log grid multisine.}%

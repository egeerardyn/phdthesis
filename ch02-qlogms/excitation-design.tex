...
Conequently, this means that for lightly-damped systems ($\damping \ll \sqrt{2}$), these derivatives will exhibit a sharp resonance peak just as the actual sub-system $G_2$ as illustrated in \figref{fig:excitation:derivatives}.
The roll-off near the resonance is determined by the damping $\damping$.
...
\begin{figure}
  ...
  \caption[Derivatives of a second-order system for each of its parameters.]{Bode plots of the elements of $\PartialDerivative{G_2}{\theta}$ for small $\damping$. 
  ...
  It can be seen that the derivatives are dominated by the resonance peak. 
  ...
  In this example, $\damping = \varXi$, $K = \varK$, $\frac{\wz}{\wn} \in \Set{\varWzSmall, 1, \varWzBig}$ and $\wn = \varWn \unit{rad/s}$ were used.
}

...
\subsection{System and noise model}
  ...
  Denote
    ...
    $Y  \left(\omega\right) = Y_0\left( \omega \right) + E\left( \omega \right)$ depicts the measured output signal as shown in \figref{fig:excitation:OE}.
...
\begin{align}
  ...
  \begin{remark}
  The use of different rounding strategies in \eqref{eq:excitation:qlog:roundAlpha}, allows for slightly different guarantees on the effective $\alpha_k$ attained by the grid.
...
\subsection{Compensated Quasi-Logarithmic Grid Multisine}
  ...
  \centering
   \caption{Summary of  properties of the different frequency grids.}
    ...
    \end{tabular}
  \end{table}
  \TODO{tabel: verschillende signalen: power per BW}
...
\TODO{zin loopt niet goed}
  ...
  For a frequency grid $\infgrid{\omega}$ with infinite span, it can be shown that $G(\infgrid{\omega}; \wn) = G(m \infgrid{\omega}; m\wn)$.
  \end{lemma}
  \begin{proof}
  Pick any $\omega_{\star} \in \infgrid{\omega}$. The frequency $m\omega_{\star}$ is then by definition an element of the grid $m\infgrid{\omega}$.
  ...
  \begin{theorem}
    ...
    Or, formally: $G(\infgrid{\omega};\wn) = G(\infgrid{\omega}; \alpha^{N}\wn) \forall N \in \NaturalNumbers$.
  \end{theorem}
  \begin{proof}

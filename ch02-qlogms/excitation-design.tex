...
\begin{guideline}[Design the logarithmic grid for the damping of the system: $\alpha \leq  1 + \xi_{\min}$]
   ...
   As such, $\alpha = 1 + \damping$ provides a nice trade-off: it has the required resolution and the best \gls{SNR} per bin.
\end{guideline}

\begin{guideline}[Ensure $\Delta \omega < (\damping - \damping^2) \wn \approx \damping \wn$]
 A quasi-logarithmic multisine with frequency resolution $\Delta \omega$ and tuned frequency spacing $\alpha$ is only guaranteed to properly excite resonances
\end{guideline}

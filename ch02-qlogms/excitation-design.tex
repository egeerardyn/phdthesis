...
\item Robust input design one tries to make the input design robust to uncertainty in the prior knowledge of the parameter values, see e.g. \citep{Rojas2007} and references therein. 
While the signal is more robust towards uncertainty in the prior knowledge, it requires more forethought and experimentation than (nominal) optimal designs.
E.g. the approach followed by \citet{Rojas2007} relies on both nominal parameter values and the distribution function of these parameters to design a robust (optimal) excitation signal.
\end{enumerate}
...
Such problem settings occur in many diverse fields of experimentation, such as the measurement of biochemical impedances\citep{Bragos2001,Sanchez2011}, electrochemical impedance spectroscopy~\citep{Niedostatkiewicz2009,VanGheem2004}, electronics\TODO{citation}, mechatronics\TODO{citation} and vibration testing\TODO{citation (ISMA?)}.
...
\section{Introduction}
  It often occurs that we wish to measure the \gls{FRF} of a \gls{LTI} system in a frequency band that covers several decades~.
...
% \TODO{begin}
% \newcommand{\robust}{\mathrm{robust}\}
...
\begin{align}
y(t) &= G(s,\theta) u(t) = \frac{1}{\frac{s^2}{\wn^2} + 2\frac{\damping}{\wn}s + 1}u(t),
...
%TODO: explain generic LTI system
Consider a second-order subsystem
\begin{equation}
  G_2(s, \theta) = \frac{K \wn^{2 \multiplicity} \left( s + \wz \right)}{\wz \left(s^2 + 2 \damping \wn s + \wn^2 \right)^{\multiplicity}}
\end{equation}
...
\begin{align}
  ...
  &= 
  \frac{2 K \wn^{2\multiplicity-1} \multiplicity}
            {\wz} 
...
Consequently, this means that for lightly-damped systems ($\damping \ll \sqrt{2}$), these derivatives will exhibit a sharp resonance peak just as the actual sub-system $G_2$ as illustrated in \figref{fig:excitation:derivatives}.
...
\section{Methodology} \label{sec:method}
  ...
  We consider the transfer function models of generic \gls{LTI} systems of arbitrary but finite orders $\order{B} / \order{A}$:
  \begin{align}
...
Remember that the rational transfer function of any strictly proper \gls{LTI} system $G$ can be decomposed in its partial fraction form:
\begin{align}
  G(s) &= 
...
\subsection{Multisine Excitations}
  ...
  For \gls{LTI} systems, it is well-known \TODO{refer to eqn. from Larsson} that the information matrix only depends upon the amplitude spectrum of the input and not upon its phase spectrum.
  As such, in this chapter assume random-phase multisines since this is a reasonable choice for systems that are dominantly linear~\TODO{referentie}.
  ...
  Note that the $k^{\text{th}}$ term in \eqref{eq:MultisineCT} is periodic with period $T_k \isdef f_k^{-1}$.
  ...
  \begin{equation}
   ...
   % more info: http://mathworld.wolfram.com/LeastCommonMultiple.html
   \end{definition}
  \begin{example}
  ...
  E.g. when $\eps \approx 1.2\cdot 10^{-7}$ (\code{single} precision), the period of a multisine with components as in Example~\ref{eg:excitation:non-periodic} is $T \approx 8.4 \cdot 10^{6} \unit{s} \approx 97 \unit{days}$.
  For \code{double} precision, $\eps \approx 2.2 \cdot 10^{-16}$ and the period $T \approx 9 \cdot 10^{6} \unit{year}$.
  \end{remark}
...
\TODO{cov-matrix uitschrijven}

\TODO{zin loopt niet goed}
  As in \citep{Pintelon2012, Gallager2008, matrixcookbook}, the covariance matrix $\C_{G\left(\omega_{\interest}\right)}$ of the parametric transfer function $G(\theta,\omega)$ can be shown to be approximately equal to
  \begin{align}
  ...
  \begin{lemma} \label{lem:exc:sysscaling}
  ...
  The arguments for a logarithmically spaced grid with an infinite span remain approximately valid for quasi-logarithmically spaced grids with a finite frequency support if the resonance peak of the system under test lies within the `bulk' of the frequency grid with finite support.
  This can be understood intuitively by inspecting the derivatives that constitute the Fisher information matrix (see \figref{fig:excitation:derivatives}): those derivatives are significantly larger near the resonance frequency than at other frequencies.
  ...
  \end{equation}
  where $\omega_k$ and $\omega_{k+1}$ are the excited grid lines closest to the frequency $\wM$ where the FRF has its peak amplitude and $\beta$ is the misalignment coefficient that is varied over the interval $\RightOpenInterval{0,1}$ as depicted graphically in \figref{fig:misalignment-logms}
...
In the following set of simulations, we will study how the FRF of different second order systems with equal damping (and the corresponding uncertainty) are measured by the multisine excitation signals from Section~\ref{sec:method}.
  ...
  \begin{remark}
  All simulations  have been repeated fitted with the \MATLAB System Identification Toolbox by means of the \mcode{oe} function~\citep{TDIDENT}.
...
For example, a system composed of two second order systems with $\damping = 0.2$ and $\fR = 400 \unit{Hz}$ for the first system and $\fR=4 \unit{kHz}$ for the second system was simulated similarly to the previous examples. The standard deviation of the noise $\sigma_e$ was put at $0.0678$. The corresponding results are  shown in \figref{fig:sumSys}.
...
\begin{figure}%
  ...
  \caption{FRF for the sum of two second order systems with $\damping=0.2$ and
           ...
           \TODO{also with other excitation signals?}
  \label{fig:sumSys}
\end{figure}

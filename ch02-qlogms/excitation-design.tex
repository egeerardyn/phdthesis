...
\subsection{System and noise model}
  In this chapter, an output error (OE) system setup is used.
  Denote
...
\subsection{Multisine Excitations}
  ...
  \begin{equation}
    \lcm \set{T_1, \ldots, T_F} \isdef
      \min \Set{T |\forall k \in \set{1,\ldots,F}, \exists n_k \in \NaturalNumbers: n_k T_k = T }
   \end{equation}
  ...
  \begin{example}
   ...
   \begin{figure}
     ...
     \input{\thisDir/figs/aperiodic.tikz}
     \caption{
     The first periods of $\sin \left( 2\pi f_1 t\right)$ with $f_1 = 1\unit{Hz}$  together with $\sin \left( 2 \pi f_2 t\right)$ for $f_2 = \pi \unit{Hz}$ to illustrate that both do not share a common period.}
     \label{fig:excitation:aperiodicmultisine}
    \end{figure}
  \end{example}

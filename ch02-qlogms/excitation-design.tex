...
\subsection{Multisine Excitations}
  ...
  \end{example}
  \begin{remark}
  In practical applications each frequency will be represented as either a fixed-point or floating point number with a limited numerical precision $\delta$.
  Hence, each frequency line $f_k$ will be approximated by a line at $\round{f_k \delta^{-1}} \delta \isdef m_k \delta$ where $m_k \in \Integers$.
  The resulting multisine has a period 
  \begin{equation}
     T 
     = \lcm \Set{T_k |  k \in \mathbb{K}} 
     = \lcm \Set{m_k^{-1} \delta^{-1} |  k \in \mathbb{K}}
     = \delta^{-1} \lcm \Set{m_k^{-1} |  k \in \mathbb{K}}
     \text{.}
  \end{equation}
  ...
  E.g. when $\delta \approx 2.2\cdot 10^{-16}$ (\code{double} precision), the period of a multisine with components as in Example~\ref{eg:excitation:non-periodic} is $T \approx 2.8 \cdot 10^{14} \unit{s} \approx 9 \cdot 10^{6} \unit{year}$.
  \end{remark}
...
\subsection{(Ideal) Logarithmic Grid Multisine}
  ...
  \end{remark}
  \begin{example}
    The~\citep{HP3562A} has a logarithmic measurement mode that ensures that each decade is covered by $F_{\decade} = 80$ lines~ to measure an \gls{FRF}.
...
\begin{align}
  ...
  \end{align}
  \end{definition}
  In the equation above, $\round{\placeholder}$ denotes rounding towards the nearest integer according to the \IEEEfloat standards and as implemented in the \MATLAB function \code{round}.
  This satisfies the approximate definition.
  
  \begin{remark}
  The use of different rounding strategies --- e.g. the \code{ceil} function $\ceil{}$  and \code{floor} function $\floor{}$ --- in \eqref{eq:excitation:qlog:roundAlpha}, allows for slightly different guarantees on the effective $\alpha_k$ attained by the grid.
  ...
  \text{.}
  \end{equation}
  This means that rounding using the \code{floor} operator favors to strictly attain the specified frequency bin density at the cost of a few more excited lines (and hence less allottable power per line).
  The \code{ceil} function produces a grid that has more power per line at the cost of a reduced bin density.
  On the other hand, using \code{round} as in \eqref{eq:excitation:qlog:roundAlpha}, provides a grid that is somewhere in-between both extremes.
  \end{remark}
  ...
  \centering
   \caption{Summary of  properties of the different frequency grids.}
    ...
    $f_{k+1} \isdef$ 
       & $f_{k} + \Delta f$ 
       & $\alpha f_{k}$
       & $\round{\frac{\alpha^{N_k} f_k}{\Delta f}} \Delta f$,   $N_k \in \NaturalNumbers$
       \\[5pt]
    ...
    $F_{\ClosedInterval{a,b}}$
       ...
       & $\leq \floor{\log_{\alpha} \frac{b}{a}}$\\[5pt]
    $F_{\ClosedInterval{f, \kappa f}}$
      & $\floor{\frac{(\kappa - 1)f}{\Delta f}}$
      & $\floor{\log_{\alpha} \kappa}$
      & $\leq \floor{\log_{\alpha} \kappa}$\\[5pt]
    \bottomrule
    \end{tabular}
  \end{table}
...
\TODO{zin loopt niet goed}
  ...
  \TODO{more rigorous explanation why this is the case}
...
In the following set of simulations, we will study how the FRF of different second order systems with equal damping (and the corresponding uncertainty) are measured by the multisine excitation signals from Section~\ref{sec:method}.
  ...
  \begin{remark}
  All simulations  have been repeated fitted with the \MATLAB System Identification Toolbox by means of the \code{oe} function~\citep{TDIDENT}.
...
\begin{subappendices}
  ...
  \TODO{benadering van 1/2 systeem a.d.h.v. 0/2 systeem wanneer $\omega_z << \wn$ (of $\omega_z >> \wn)$: rond de resonantie zijn beiden ongeveer gelijk. Afgeleide naar de nul zijn klein rond de resonantie. Informatiematrix is langs twee kanten gewogen met die afgeleide, dus die nul maakt weinig uit.}
\end{subappendices}

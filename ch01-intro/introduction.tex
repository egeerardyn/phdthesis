...
In particular, \gls{LTI} systems are a first step for many people to build a system model.
...
When such a linear model is not adequate, a more flexible model, e.g. a time-varying model, a nonlinear model or parameter-varying model needs to be constructed.
However, many of those advanced modeling approaches build upon \gls{LTI} models.
As such, improvements in estimating \gls{LTI} models also indirectly benefit more advanced models.

...
The $i^{\text{th}}$ segment is denoted $\segm{\hat{g}}{i}$, for $i \in \left\{1,\ldots,N_S\right\}$.
  ...
  \end{equation}
  where $ \ICDF{\FDistribution}$ is the inverse \gls{CDF} of an $F$ distribution with $n_{i-1}$ and $n_{i}$ degrees of freedom.
...
\caption[Illustration of impulse response truncation using the $F$-test.]{Illustration of truncating the impulse response by performing an $F$-test on successive segments of the impulse response. The different segments are separated by dotted lines.}

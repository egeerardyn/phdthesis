...
\subsubsection{The Hanning Window and the LPM}\label{se:LPMFRFest}%\label{se:FRFhanningLPM}
\paragraph{Windowing}
...
\item
A truncation is effected at the point beyond which the data record (impulse response) is buried in noise. % main part lies at the beginning, and one can assume that, from a certain time instant, only noise remains.)
...
\subsubsection{Estimating the DC Value of the FRF}\label{se:DCvalueEst}
...
The FRF estimate is smoothed by truncating the estimated impulse response function. The truncation is applied at the time index beyond which the signal is indistinguishable from the noise. %The truncation is applied at the time index after which the signal is expected not to be distinguishable from the noise
...
%
%
%\subsubsection{Truncation by Fitting an Exponential on the Maxima}
%\label{se:expFitTheory}
%
%An alternative truncation method is to determine the index at which the envelope of the estimated impulse response 
%%at which the estimated impulse response 
%gets below the noise floor.
%%Another way of truncating is by determining from which index on the envelope of the estimated impulse response gets below the noise floor.
%This algorithm has been implemented as follows. 
...
%\JL{
%TO BE FURTHER ELABORATED: what is `significantly' below the noise floor?? Truncation reduces the variance of the estimate, but blows up the bias error. The impulse response should be truncated at that index where the bias error becomes approximately equal to the standard deviation of the truncated estimate. Can this trade off be computed experimentally? 
%}

...
Since it is well-known that the Vandemonde structure in the design matrix \eqref{eq:nparam:designMatrix} easily leads to ill-conditioned systems, additional measures are taken to improve numerical conditioning.
...
\begin{equation}
  \Delta\theta = - \left(   \PartialDerivative[2]{\ELSCost{\true\theta}}{\true\theta^2} \right)^{-1}  \left( \PartialDerivative{\ELSCost[n]{\true\theta}}{\true\theta} \right)^{\T}
  \text{.}
\end{equation}
...
Particularly, the function $\ELSCost{\theta}$ is regarded as a two-dimensional function of $\ELSCost{\theta,\conj{\theta}}$ where $\conj{\theta}$ and $\theta$ are considered as independent variables which will be derived with respect to the real and imaginary part of $\theta$.
...
Consequently, \citep[Equation 15-60]{Pintelon2012}:
\begin{equation}
  \left(
     \PartialDerivative{\ELSCost{\theta,\conj{\theta}}}{\ReImTheta}
  \right)^{\TT}
  = 
  2 
  \ReIm{ \left(
        \left(
            \PartialDerivative{\ELSCost{\theta,\conj{\theta}}}{\theta}
        \right)^{\HT}
    \right) }
  \text{.}
\end{equation}
To facilitate the derivations, we rewrite the expected cost functions in terms of $\theta$ and $\conj{\theta}$:
\begin{align}

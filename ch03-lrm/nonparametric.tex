...
\end{itemize}
The main terms of interest are the transfer function $\true{G}$ and the noise variance $\sigma^2_{V}$.
...
On the other hand, it is well-known that the transfer functions $\true{G}$, $\true{H}$ and hence also the corresponding transient contributions $T = T_G + T_H$ are smooth over the frequency (see \appref{app:nparam:leakage}).
...
\begin{figure}[p]
  ...
  \caption[Comparison of local models for $\SNR = 60 \unit{dB}$]{Comparison of bias and variance of local modeling for $\SNR=60\unit{dB}$. \\
...
\begin{figure}[p]
  ...
  \caption[Comparison of local models for $\SNR = 40 \unit{dB}$]{Comparison of bias and variance of local modeling for $\SNR=40\unit{dB}$. \\
  The shaded area denotes the noise floor.}
...
\begin{figure}[p]
 ...
 \caption[Comparison of local models for $\SNR = 10 \unit{dB}$]{Comparison of bias and variance of local modeling for $\SNR=10\unit{dB}$. \\
 The shaded area denotes the noise floor.}
...
y(t)=\true{G}(q)\true{u}(t)+\true{H}(q)e(t)=\true{G}(q)\true{u}(t)+v(t)
\end{equation}
where $\true{G}(q)$ represents the dynamics of the system to be estimated, $\true{u}(t)$ is the input signal, $v(t)= \true{H}(q)e(t)$ is the noise source at the output, $\true{H}(q)$ is the noise dynamics, $e(t)$ is white noise, and $q^{-m}$ is the backward shift operator ($q^{-m}x(t)$ = $x(t-m)$  with $m$ a positive integer).
...
These \gls{FRF} estimates are also compared with the true \gls{LTI} system $\true{G}(\omega_k)$.
...
% \begin{assumption}
% The measured output signal is $y(t) = \true{y}(t) + v(t)$ with the exact input signal $\true{u}(t)$ known. 
...
\begin{align}
\true{y}(t)  &= 1.5371\true{y}(t-1)    -0.9025 \true{y}(t-2) + u(t)\\
y(t)       &= \true{y}(t) + e(t),
\end{align}
\end{subequations}
...
\begin{equation}
\true{y}(t) = 2.583\true{y}(t - 1) -2.176\true{y}(t - 2)+0.592\true{y}(t-3) + u(t)
\end{equation}

The measured signal was disturbed by random white noise, $y(t) = \true{y}(t) + e(t)$, such that the \gls{SNR} was $14.2\unit{dB}$.
